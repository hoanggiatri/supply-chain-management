...
const NAV_ITEMS = [
  ...
  { title: "Danh sách sản phẩm", path: "/products", keywords: "san pham product list" },
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <div className="hidden md:block w-64 lg:w-96 relative" ref={searchRef}>
              <div className="relative">
                <Input
                  ...
                  className="!border-blue-gray-200 dark:!border-dark-border focus:!border-blue-500 !text-blue-gray-700 dark:!text-dark-text bg-blue-gray-50/50 dark:bg-dark-bg/50 pl-10 pr-4 py-2 h-[40px] rounded-lg transition-all duration-300 focus:w-full focus:bg-white dark:focus:bg-dark-surface focus:shadow-lg"
                  labelProps={{ className: "hidden" }}
                  containerProps={{ className: "min-w-[200px]" }}
                />
                <MagnifyingGlassIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 h-5 w-5 text-blue-gray-400 group-focus-within:text-blue-500 transition-colors" />
              </div>

              {/* Search Results Dropdown */}
              {showResults && searchResults.length > 0 && (
                <Card className="absolute top-full left-0 right-0 mt-2 max-h-96 overflow-y-auto z-50 shadow-xl border border-blue-gray-100 dark:border-dark-border dark:bg-dark-surface animate-scale-in">
                  <List className="p-1">
                    {searchResults.map((item, index) => (
                      <ListItem 
                        key={index} 
                        onClick={() => handleNavigate(item.path)}
                        className="hover:bg-blue-50 dark:hover:bg-white/10 focus:bg-blue-50 dark:focus:bg-white/10 rounded-md"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <Typography variant="small" className="font-semibold text-blue-gray-900 dark:text-dark-text">
                              {item.title}
                            </Typography>
                            <Typography variant="small" className="text-[10px] text-gray-500 dark:text-dark-muted">
                              Điều hướng
                            </Typography>
                          </div>
                        </div>
            ...
            <Menu placement="bottom-end">
              <MenuHandler>
                <div className="flex items-center gap-3 cursor-pointer hover:bg-blue-gray-50 dark:hover:bg-white/10 rounded-full p-1 pr-3 transition-colors border border-transparent hover:border-blue-gray-100 dark:hover:border-dark-border">
                  <Avatar
                    src={`https://ui-avatars.com/api/?name=${userName}&background=2563eb&color=fff`}
                    alt={userName}
                    size="sm"
                    className="border-2 border-white dark:border-dark-surface shadow-sm"
                  />
                  <div className="hidden lg:block text-left">
                    <Typography variant="small" className="font-semibold text-blue-gray-900 dark:text-dark-text leading-tight">
                      {userName}
                    </Typography>
                    <Typography
                      variant="small"
                      color="gray"
                      className="text-xs dark:text-dark-muted font-normal"
                    >
                      {userEmail}
                    </Typography>
                  </div>
