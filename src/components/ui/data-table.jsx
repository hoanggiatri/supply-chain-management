...
export function DataTable({
  ...
  exportMapper = (row) => row, // map dữ liệu khi export
}) {
  ...
  const getPageNumbers = () => {
    ...
    for (
      ...
      i <= Math.min(pageCount - 1, pageIndex + delta + 1);
      i++
    ) {
      range.push(i);
    }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col h-full overflow-hidden">
      ...
      <div className="p-5 border-b border-gray-100 bg-white flex justify-between items-center gap-4">
        ...
        <button
          onClick={() => {
            const mapped = (data || []).map(exportMapper);
            const dataset = mapped.length > 0 ? mapped : [exportMapper({})];
            exportToExcel(dataset, exportFileName);
          }}
                ...
                return (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td
                        key={cell.id}
                        className="px-6 py-4 text-sm font-normal text-gray-900 whitespace-nowrap"
                      >
                        {flexRender(
              ...
              <tr>
                <td colSpan={columns.length} className="p-12 text-center">
                  <div className="flex flex-col items-center justify-center min-h-[200px] text-gray-400">
                    ...
                    {globalFilter && (
                      <p className="text-sm">
                        Không tìm thấy kết quả cho "{globalFilter}"
                      </p>
                    )}
                  </div>
                </td>
